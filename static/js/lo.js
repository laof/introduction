(function () {
    ...
    lo.isObject = function (obj) {
        return typeString(obj) == 'string';
    }
    lo.isArray = function (obj) {
        return typeString(obj) == 'array';
    }
    lo.isFunction = function (obj) {
    ...
    function getRequest() {
        ...
        lo.pageData = $.extend({}, lo.pageData, theRequest);
        return lo.pageData;
    };
    getRequest();
    lo.bootstrap = function (opts) {
        lo.opts = $.extend({}, lo.opts, opts);
        var router = lo.pageData.router || this.opts.defaultRouter;
        lo.load(router);
    };
    lo.download = function (api, data, method) {
        var url = lo.url(api);
    ...
    function promise(options) {
        ...
        options.data.token = lo.pageData.token;
        var defaultOptions = {
    ...
    lo.get = function (url, data) {
        url = lo.url(url);
        return promise({ type: 'get', url: url, data: data });
    }
    lo.post = function (url, data) {
        url = lo.url(url);
    ...
    Select.prototype.update = function (array) {
        if (array) {
    ...
    lo.Search = function (selector, options) {
        var than = this;
        this.ele = $(selector);
